...
async def signup_for_ai_coach_notifications(request: NotificationSignupRequest) -> NotificationSignupResponse:
    ...
    try:
        ...
        doc_ref = db_firestore.collection("ai_coach_notifications").document(request.email)
        ...
        signup_id = f"signup_{int(datetime.now().timestamp())}"
        signup_data = {
            "email": request.email,
...
async def get_notification_stats() -> GetNotificationStatsResponse:
    ...
    try:
        ...
        for doc in db_firestore.collection("ai_coach_notifications").stream():
            data = doc.to_dict()
            if not data:
                continue
            total_signups += 1
