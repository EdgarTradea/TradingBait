...
async def early_access_signup(request: EarlyAccessSignupRequest) -> EarlyAccessSignupResponse:
    ...
    try:
        ...
        # Check if email already exists (use email as document ID)
        doc_ref = db_firestore.collection("early_access").document(email)
...
async def get_early_access_stats():
    ...
    try:
        db_firestore = firestore.client()
        docs = db_firestore.collection("early_access").stream()
